...
def error(args):
    ...
    with sd.SimulationHDF(args.file) as file:
        ...
        for j, phi_index in enumerate(args.c):
            ...
            if (
                "scri" in 
                list(sims[-1].raw.group[str(sims[-1].indexOfTime(args.t))].attrs.keys())
                ):
                ...
                while nextFrame_index<stop_index+1:
                    ...
                    if args.o and args.frame is None:
                        for add_data, add_dg in additional_data:
                            plotItems += [Gnuplot.Data(domains[i],add_dg[i],\
                                title = add_data,\
                        ...
                        for add_data, add_dg in additional_data:
                            plotItems += [Gnuplot.Data(domains[i],add_dg[i],\
                                ...
                                (args.ofile_base, add_data,
                                    sim.name, args.t, args.frame))]
                    else:
                        for add_data, add_dg in additional_data:
                            plotItems += [Gnuplot.Data(domains[i],add_dg[i],\
                                title = add_data)]
...
"""This program allows different types of display 
of data contained in hdf files generated
using simulation_data.py.""")
subparsers = parser.add_subparsers(title='Subcommands',description="""The hdfvis command allows for several different type of graphs and 
animations of hdf files produced by simulation_data.py hdf file. Each of the subcommands allows access to a particular type of graph or animation.""",help=\
"""For more information on any of the subcommands please use 'hdfvis <subcommand> -h""")
...
"""The data group type to be plotted. See simulation_data for information on the different data groups. Defaults to "raw". 
Multiple data types can be given by specifying multiple '-dg' options.""")

parser.add_argument('-d','-display',action='store_true',default=False,help=\
"""This flag indicates that the visualisations are to be displayed on screen. No files will be saved. 
...
parser.add_argument('-g', '-gnuplot',metavar='COMMAND',action='append',help=\
"""A list of gnuplot commands. These commands are provided to a Gnuplot.py object. If not specified the defaults are read in from 'hdfvis_gnuplot_defaults.py'. 
This file can be editted to change the defaults. Multiple gnuplot commands can be given by specifying multiple '-g' commands.""")

parser.add_argument('-o','-output',action="store_true",default=False,help=\
"""A flag to indicate that the data used for the plot is to be written to the HDD.""")

parser.add_argument('-add', '-additional_data', default = [], \
    action = 'append', help = \
"""A list of data types for plotting in addition to the data specified by the
...
################################################################################
parser_ani = subparsers.add_parser('ani', help=\
"""This command produces an animation of the data over the given times against the relevant domain."""\
)
parser_ani.set_defaults(func=anima)
parser_ani.add_argument('-s','-sims', type=int,metavar='INDEX',nargs='+',\
help=\
"""A list of the simulations to display information for given by the index of the simulation in the sorted list of simulation given by CMP. 
If not set an animation or graph for each simulation is given. """)
parser_ani.add_argument('-t0','-tstart',metavar='TIME',\
    default=-float('Infinity'),\
    type=float, help=\
"""The start time of the animation or plot. Defaults to the earlist possible time.""")
parser_ani.add_argument('-t1','-tstop',type=float,metavar='TIME',\
default=float('Infinity'), help=\
"""The stop time of an animation. Defaults to the latest possible time.""")

################################################################################
# Parser for plots
################################################################################
parser_plot = subparsers.add_parser('plot', help=\
"""The subcommand 'normal' produces a graph or animation (based on -t0 and -t1) of the values of functions at a particular time 
(or across a range of times for an animation) against the domain of those functions""")
parser_plot.set_defaults(func=plot)
parser_plot.add_argument('-s','-sims', type=int,metavar='INDEX',nargs='+',\
help=\
"""A list of the simulations to display information for given by the index of the simulation in the sorted list of simulations given by increasing CMP. 
If not set an animation or graph for each simulation is given. """)
parser_plot.add_argument('-t','-time',metavar='TIME',type=float,help=\
"""The time at which the data will be plotted.""")

################################################################################
# Parser for error graphs
################################################################################
parser_error= subparsers.add_parser('err',help=\
"""The subcommand 'error' plots the precalculated error of a particular component of a particular data type against the domain of the component at a particular time. 
...
parser_error.add_argument('-c','-components',nargs='+',type=int,help=\
"""A list of the components for which you want error plots produced. If not set a graph for each component will be made.""")
parser_error.add_argument('-t','-time',metavar='TIME',type=float,help=\
"""The time at which the errors for the given component should be displayed.""")
parser_error.add_argument('-e','-exact',action="store_true",default=False,help=\
"""A flag that indicates that the errors calculated against the exact data
should be graphed.""")
