...
def valid(j1, j2, j3, m1, m2, m3):
    ...
    if not (2 * j1 % 1 < NUMERICAL_ERROR_TOLERANCE and  
        2 * j2 % 1 < NUMERICAL_ERROR_TOLERANCE and
...
def trivial_zero(j1, j2, j3, m1, m2, m3):
    ...
    """
    WI = j1+j2+j3
    if not WI >= 0 and not WI%1 < NUMERICAL_ERROR_TOLERANCE: return True
    WII = abs(m1+m2+m3)
    if not WII < NUMERICAL_ERROR_TOLERANCE: return True
...
class W3jBen(object):
    ...
    def __call__(self, j1, j2, j3, m1, m2, m3):
        ...
        if data_set is not None:
            ...
            w3j_arr, j1_min, j1_num, j1_max = self._compute_values(
                j2, j3, m1, m2, m3
                )
    ...
    def _write_w3j_symbols(
        ...
        for i in range(0, j1_num):
            ...
            if data_set is None:
                ...
                if odd_flip and J%2 == 1:
                    data = -data
                data_set = self.hdf_file.create_dataset(
                    repr(regge_hash), data=[data]
                )
            js[0] = js[0] + 1
...
class W3jBoris(object):
    ...
    def __init__(self, L, dl):
        ...
        self._cg.calcw3jSymbols.argtypes  = [
        ...
        self._cg.checkCw3jStructExists.argtypes = [
            ctypes.c_double, ctypes.c_double
        ]
            
        self._cg.readcplCw3jStruct.restype = ctypes.c_void_p
        self._cg.readcplCw3jStruct.argtypes = [
            ctypes.c_double, ctypes.c_double
        ]
