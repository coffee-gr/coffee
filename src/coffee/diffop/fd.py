...
class FD_diffop(object):
    ...
    def __call__(self, u, dx):
        ru     = self.central(u)
        if (be.name == "jax"):
            ru = ru.at[0].set(self.left_boundary(u, apply_at=0))
            ru = ru.at[-1].set(self.right_boundary(u, apply_at=-1))
        else:
            ru[0]  = self.left_boundary(u, apply_at=0)
            ru[-1] = self.right_boundary(u, apply_at=-1)
...
class FD12(FD_diffop):
    ...
    central = C12_stencil()
    left_boundary  = F12_stencil()
    right_boundary = B12_stencil()
    order = 1