...
parser.add_argument('-s','-simulation', help=\
"""The simulation to be run. If not specified -f must be used.""")
...
if args.terrN_bash is not '':
    ...
    errorNum_run += ["python -O '%s' %s %s -Lp '2' numer '%s'"%\
            (command, args.terrN_bash, args.dg, args.f)]
        
hdfvis_error_num = []
command = os.path.join(path,"visualise.py")
for time in args.terrN:
    hdfvis_error_num += ['python "%s" %s err -t "%s" "%s"'%\
        (command, args.dg, time, args.f)]
        
errorExa_run = []
if args.terrE_bash is not '':
    command = os.path.join(path,"error.py")
    errorExa_run += ["python -O '%s' %s %s -Lp '2' exact '%s'"%\
            (command, args.terrE_bash, args.dg, args.f)]
        
hdfvis_error_exa = []
command = os.path.join(path,"visualise.py")
for time in args.terrE:
    hdfvis_error_exa += ['python "%s" %s err -e -t "%s" "%s"'%\
        (command, args.dg, time, args.f)]

hdfvis_plot = []
command = os.path.join(path,"visualise.py")
for time in args.tplo:
    hdfvis_plot  += ['python "%s" %s plot -t "%s" "%s"'%\
        (command, args.dg, time, args.f)]
        
hdfvis_ani = []
command = os.path.join(path,"visualise.py")
for tani in args.tani:
        hdfvis_ani  += ['python %s "%s" ani -t0 "%s" -t1 "%s" "%s"'%\
